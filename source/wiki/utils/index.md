...
## 重复
; 重复最近的字符查找（f、t 等）操作, 重复最近的字符查找操作
， 反方向的;
n  重复最近的字符串查找操作（/ 和 ?）
...
diw:  （delete inside words）删除整个单词文本，但是保留空格字符不删除
d2w:  删除从当前光标开始处的2个单词
d$:  删除从光标到一行末尾的整个文本
d0:  删除从光标到一行开头的所有单词
dl:  删除当前光标处的字符=x
...
?string   反向查找
:s 进入替换模式
https://yianwillis.github.io/vimcdoc/doc/pattern.html#pattern-overview
https://yianwillis.github.io/vimcdoc/doc/change.html#:s_flags
## 设置高亮显示
:set hls
